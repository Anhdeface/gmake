...
- **Parser (`internal/parser`)**: Reads `.gomake` files line by line, excludes inline comments starting with `//`, and extracts keys and values from the `[config.name]` and `[config.dependency]` blocks. The parsing execution halts upon encountering the `./gomake` EOF marker.
...
- `[config.name]`: Stores compiler designation, compilation flags, and the project name.
- `[config.dependency]`: Stores target output definitions, source paths (supporting wildcard globbing), include directories, and object linking states (`o.fix`).
